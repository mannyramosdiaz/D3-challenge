...
var chartGroup = svg.append("g")
  ...
  function xScale(data, xAxis) {  
    var xLinearScale = d3.scaleLinear()
      .domain([d3.min(data, d => d[xIs])-1,d3.max(data, d => d[xIs])+1])
      .range([0, width]);
    return xLinearScale;
  }
  function yScale(data, yAxis) {
    var yLinearScale = d3.scaleLinear()
      .domain([d3.min(data, d => d[yIs])-1, d3.max(data, d => d[yIs]+1)])
      .range([height, 0]);
    return yLinearScale;
  }
...
// new circles
function renderCircles(circlesGroup, newX, xIs,newY, yIs) {
    circlesGroup.transition()
      .duration(1000)
      .attr("cx", xx => newX(xx[xIs]))
      .attr("cy", yy => newY(yy[yIs]));
  
    return circlesGroup;
}
// Function to render state abbreviation 
function renderStateAbbr(stateAbbr, newX, xIs,newY, yIs) {
    
    stateAbbr.transition()
    .duration(1000)
    .attr("x", xxx => newX(xxx[xIs]))
    .attr("y", yyy => newY(yyy[yIs]));
    return stateAbbr;
}

// Function used to update circles group with new tooltip
function updateToolTip(xIs, yIs, circlesGroup) {
  if (xIs == "poverty"){
      var labelx = "Poverty: ";      
  }
  else if(xIs == "age") {
  ...
  // Step 6: Initialize tool tip
    ...
    .style("font-size", "8px")
    .html(function(tip){return(`${tip.state}<br>${labelx} ${formatAxis(tip[xIs], xIs)} <br> 
    ${labely} ${formatAxis(tip[yIs], yIs)} `) // 
    });
...
function formatAxis(axisValue, xIs, yIs) {
...
data.forEach(function(data) {
  ...
  let xLinearScale = xScale(data, xIs);
  let yLinearScale = yScale(data, yIs);
...
// append initial circles
  ...
  .attr("cx", xx => xLinearScale(xx[xIs]))
  .attr("cy", yy => yLinearScale(yy[yIs]))
  .attr("r", 12)
  // .attr("fill", "#00008B")
  .attr("opacity", "0.8");
...
labelsGroupX.selectAll("text")
    ...
    // get value of selection
        ...
        if (xValue !== xIs) {
            ...
            xLinearScale = xScale(data, xIs);
            // updates x axis with transition
            xAxis = renderAxesX(xLinearScale, xAxis);
            // updates circles with new x values
            circlesGroup = renderCircles(circlesGroup, xLinearScale, xIs, yLinearScale, yIs);
            
            // updates tooltips with new info
            circlesGroup = updateToolTip(chosenXAxis,chosenYAxis, circlesGroup);
            // updates state abbreviate with new info
            stateAbbr = renderStateAbbr(stateAbbr, xLinearScale, chosenXAxis, yLinearScale, xxIs); 
